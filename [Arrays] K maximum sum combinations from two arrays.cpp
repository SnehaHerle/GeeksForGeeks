...
{ 
	...
	// Max heap which contains tuple of the format (sum, (i, j)) i and j are the indices 
	// of the elements from array A and array B which make up the sum. 
	
	priority_queue<pair<int, pair<int, int> > > pq; 

	// my_set is used to store the indices of the pair(i, j) we use my_set to make sure 
	...
	{ 
		...
		// insert only if the pair (i - 1, j) is not already present inside the map i.e. 
		...
		// insert only if the pair (i, j - 1) is not present inside the heap. 
